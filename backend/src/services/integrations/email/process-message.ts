...
function parseMetadata(event: ConnectorEvent): EmailEventMetadata {
  return (event.metadataJson as EmailEventMetadata | null) ?? {};
}
...
/**
 * Gated email processing: skip heuristics → keyword filter → AI extraction queue.
 */
export async function processEmailMessage(
  ...
  if (skip.skip) {
    await EventModel.markProcessed(event.id);
    logger.debug(
      { eventId: event.id, reason: skip.reason },
      `${logPrefix}: skipped`,
    );
    return;
  }
  ...
  logger.debug({ eventId: event.id }, `${logPrefix}: enqueued AI extraction`);
}
